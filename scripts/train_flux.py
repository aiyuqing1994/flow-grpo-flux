...
ID_COLUMN = "id"
IMAGE_CACHE_DIR = "data/images_tmp"
...
class ImageEditDataset(Dataset):
    def __init__(self, csv_path, split='train', resolution=768, test_count=16, image_cache_dir=IMAGE_CACHE_DIR):
        ...
        self.target_shape = (resolution, resolution)
        ...
        if split == 'train':
            self.rows = all_rows[:-self.test_count]
        else:
            self.rows = all_rows[-self.test_count:]
    ...
    def __getitem__(self, idx):
        ...
        image = self._get_image_from_cache(image_url)
        prompt = self._construct_prompt(row)
        text = self._get_text(row)

        return {"prompt": prompt, "image": image, "text": text, "metadata": {"id": image_id}}

    def _get_image_from_cache(self, image_url):
        """
        Downloads an image from a URL and caches it locally.
        The filename is extracted directly from the URL's path.
        """
        if not image_url:
            return None
        ...
        try:
            ...
            print(f"Warning: Could not parse URL, skipping: {image_url}")
            return None
    ...
    def collate_fn(examples):
        ...
        # Preprocessing is now handled in the main loop before VAE encoding
        return images, prompts, texts, metadatas
...
def eval(pipeline, test_dataloader, text_encoders, tokenizers, config, accelerator, global_step, reward_fn, executor,
         autocast, num_train_timesteps, ema, transformer_trainable_parameters):
    if config.train.ema:
    ...
    for test_batch in tqdm(
        ...
        input_images, prompts, texts, prompt_metadata = test_batch
        ...
        if len(prompt_embeds) < len(sample_neg_prompt_embeds):
            ...
            input_images = input_images[:len(prompt_embeds)]

        if input_images:
            input_images = np.stack([np.array(img) for img in input_images])
            input_images = torch.from_numpy(input_images).float() / 255.0
    ...
    if accelerator.is_main_process:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            sampled_rewards = [{k: last_batch_rewards_gather[k][index] for k in last_batch_rewards_gather} for index in
                               sample_indices]
...
def main(_):
    ...
    train_dataset = ImageEditDataset(config.dataset, split='train', resolution=config.resolution)
    test_dataset = ImageEditDataset(config.dataset, split='test', resolution=config.resolution)
    ...
    train_dataloader = DataLoader(
        ...
        num_workers=1,
        collate_fn=ImageEditDataset.collate_fn,
    ...
    while True:
        ...
        for i in tqdm(
            ...
            input_images, prompts, texts, prompt_metadata = next(train_iter)
            ...
            rewards = executor.submit(reward_fn, output_images, texts, prompt_metadata, only_strict=True)
            # yield to to make sure reward computation starts
            time.sleep(0)
        ...
        if epoch % 10 == 0 and accelerator.is_main_process:
            ...
            with tempfile.TemporaryDirectory() as tmpdir:
                num_samples = min(15, len(output_images))
            ...
            for i, sample in tqdm(
                ...
                for j in tqdm(
                    ...
                    with accelerator.accumulate(transformer):
                        with autocast():
                            prev_sample, log_prob, prev_sample_mean, std_dev_t = compute_log_prob(transformer, pipeline,
                                                                                                  sample, j, embeds,
